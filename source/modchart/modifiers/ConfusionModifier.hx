...
class ConfusionModifier extends Modifier
{
	override function updateNote(note:Note, player:Int, pos:Vector3, scale:FlxPoint)
	{
		...
		if (!note.isSustainNote)
			note.angle = (getPercent(player) + getSubmodPercent('confusion${note.noteData}',
	...
	{
		receptor.angle = (getPercent(player)
			+ getSubmodPercent('confusion${receptor.noteData}', player)
			+ getSubmodPercent('receptor${receptor.noteData}Angle', player)) * 100;
	}
