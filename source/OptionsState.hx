...
class PreferencesSubstate extends MusicBeatSubstate
{
	...
	static var noCheckbox:Array<String> = ['Framerate', 'Note Delay', 'Scroll Speed', 'Note Size'];
				...
				if (holdTime > 0.5 || controls.UI_LEFT_P || controls.UI_RIGHT_P)
					switch (options[curSelected])
					{
						...
						case 'Scroll Speed':
							...
							if (ClientPrefs.speed < 0.5)
								ClientPrefs.speed = 0.5;
							else if (ClientPrefs.speed > 4)
								ClientPrefs.speed = 4;

						case 'Note Size':
							ClientPrefs.noteSize += add / 20;
							if (ClientPrefs.noteSize < 0.5)
								ClientPrefs.noteSize = 0.5;
							else if (ClientPrefs.noteSize > 1.5)
								ClientPrefs.noteSize = 1.5;
