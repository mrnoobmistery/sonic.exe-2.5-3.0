...
class ChartingState extends MusicBeatState
{
	public static var noteTypeList:Array<String> = // Used for backwards compatibility with 0.1 - 0.3.2 charts, though, you should add your hardcoded custom note types here too.
		[
			'', 'Alt Animation', 'Hey!', 'Phantom Note', 'Hurt Note', 'Static Note', 'Majin Note', 'Pixel Note', 'Hex Note', 'GF Sing', 'No Animation'
		];
	...
	var eventStuff:Array<Dynamic> = [
		...
		[
			'tailsjump',
			"i have been coding triple trouble for 7 FUCKING HOURS\nthis is a new form of torture i swear"
		],
		[
			'knuxjump',
			"im doing the jumpscares then going to bed FUCK YOU its 3am and ive been doing this since 4pm"
		],
		[
			'eggjump',
			"eggballs ahgahhgahhaAGHAGHAHGAGHAHWAHGHGASDGHJWERSAGSHEJDJGHAGFGHADSHFGAGHDSFGAHFDSGHDFSHGSFDGHSFGHDSFHGAFGDGDFHASAGHFDHFSDHGFAFGHSDHFSD"
		],
		[
			"Fatality Popup",
			"Fatality Popup WOo.\nValue 1: Amount of popups\nValue 2: Type (1, 2 or 3)\nType 2 ignores the hard cap of " + FatalPopup.limit +
			" popups\nand are also a bit bigger\nType 3 is the same but alot bigger"
		],
		["Clear Popups", "Removes all fatality popups"],
		[
			"Chroma Video",
			"Video that chromakeys out green\nValue 1: MP4 name\nThese won't show with flashing lights off because\nthis is mostly used for MLG shit!!"
		],
		[
			"Lyrics",
	...
	override function update(elapsed:Float)
	{
		...
		curRenderedNotes.forEachAlive(function(note:Note)
		{
			...
			if (note.strumTime <= Conductor.songPosition)
			{
				...
				else if (note.strumTime > lastConductorPos
					&& ((playSoundBf.checked && note.mustPress) || (playSoundDad.checked && !note.mustPress))
					&& FlxG.sound.music.playing
					&& note.noteData > -1)
				{
